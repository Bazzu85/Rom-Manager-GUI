# from https://github.com/pycontribs/tendo/blob/main/src/tendo/singleton.py
...
class SingleInstance:
    ...
    def __init__(self, flavor_id="", lockfile=""):
        ...
        global_variables.logger.debug(f"SingleInstance lockfile: {self.lockfile}")
    ...
    def __enter__(self):
        if sys.platform == "win32":
            try:
                ...
                if e.errno == 13:
                    global_variables.logger.error("Another instance is already running, quitting.")
            ...
            try:
                ...
                global_variables.logger.warning("Another instance is already running, quitting.")
                raise SingleInstanceException()
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        ...
        if exc_value is not None:
            global_variables.logger.warning("Error: %s" % exc_value, exc_info=True)
        try:
            if sys.platform == "win32":
                if hasattr(self, "fd"):
                    os.close(self.fd)
