...
def add_extension():
    ...
    if ui_extension_input.error != None or ui_extension_input.value == '':
        ...
        if already_inserted:
            ...
            global_variables.configuration.extensions_for_file_move.append(ui_extension_input.value)
...
def delete_extensions():
    ...
    if len(ui_extension_table.selected) > 0:
        ...
        for selected_row in ui_extension_table.selected:
            ...
            for i in range(len(global_variables.configuration.extensions_for_file_move) - 1, -1, -1):
                if global_variables.configuration.extensions_for_file_move[i] == extension_to_remove:
                    del global_variables.configuration.extensions_for_file_move[i]
