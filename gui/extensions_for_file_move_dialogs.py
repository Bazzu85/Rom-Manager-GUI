...
def edit_extensions_for_file_move_dialog():
...
def add_extension():
    ...
    if ui_extension_input.error != None or ui_extension_input.value == '':
        ...
        if already_inserted:
            ...
            globals.configuration.extensions_for_file_move.append(ui_extension_input.value)
...
def delete_extensions():
    ...
    if len(ui_extension_table.selected) > 0:
        ...
        for selected_row in ui_extension_table.selected:
            ...
            for i in range(len(globals.configuration.extensions_for_file_move) - 1, -1, -1):
                if globals.configuration.extensions_for_file_move[i] == extension_to_remove:
                    del globals.configuration.extensions_for_file_move[i]
...
def close_dialog(result):
    globals.logger.debug(inspect.currentframe().f_code.co_name)
    globals.ui_config_extensions_for_file_move_dialog.submit(result)
